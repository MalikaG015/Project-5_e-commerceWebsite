...
const getCart = async function (req, res) {
  try {
    ...
    return res.status(200).send({ status: true, msg: " your cart details are", data: cartData })
  }
  catch (error) {
    return res.status(500).send({ status: false, msg: error.message })
  }
...
const deleteCart = async function (req, res) {
  try {
    ...
    return res.status(204).send({ status: true, message: ` cart removed successfully` })
